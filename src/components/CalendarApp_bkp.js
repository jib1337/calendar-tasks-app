...
const CalendarApp = () => {
  ...
  return (
    <div className="p-9 w-full h-full mx-auto flex gap-4">
      <div className="w-1/5">
        ...
        <div className="mb-4">
          ...
          <Input
            ...
            className="mb-2 text-base border border-gray-300 rounded"
          />
          <textarea
            placeholder="Enter description"
            value={descriptionInput}
            onChange={(e) => setDescriptionInput(e.target.value)}
            className="mb-2 text-base border border-gray-300 rounded w-full p-2 resize-y"
            rows="23"
          ></textarea>
      ...
      <div className="flex-1">
        ...
        <div className="grid grid-cols-7 gap-2 mb-4">
          {calendar.map((day, index) => (
            <div
              key={index}
              className={`border p-2 min-h-24 h-auto overflow-y-auto ${day.date === today ? 'bg-yellow-200' : ''}`}
            >
              {day.date && (
                <div>
                  ...
                  <div className="text-xs space-y-1">
                    {(tasks[day.date] || []).map((task, idx) => (
                      <div
                        key={idx}
                        className={`p-1 rounded cursor-pointer ${task.completed ? 'bg-green-200 line-through' : 'bg-blue-100'}`}
